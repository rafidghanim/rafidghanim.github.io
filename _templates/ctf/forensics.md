---
layout: post
title: "[CTF Name] - [Challenge Name]"
description: "Short summary of the challenge and solution."
date: YYYY-MM-DD HH:MM:SS +0700
categories: [writeup, ctf]
tags: [ctf, forensics]
ctf: "CTF Name"
challenge: "Challenge Name"
category: "Forensics"
difficulty: "Easy/Medium/Hard"
points: 0
author: "gr3yr4t"
toc: true
comments: true
image: /assets/img/avatar.jpg
---

# {{ page.title }}

## TL;DR

Brief summary of the bug, vulnerability, trick, or core idea.

## Challenge Information

| Field | Value |
|---|---|
| CTF | {{ page.ctf }} |
| Challenge | {{ page.challenge }} |
| Category | {{ page.category }} |
| Difficulty | {{ page.difficulty }} |
| Points | {{ page.points }} |

## Description

Paste the original challenge description here.

## Files

List provided files, URLs, binaries, attachments, or source code.

## Initial Analysis

Explain first observations and assumptions.

## File Identification

Record file types, magic bytes, archive structure, and suspicious embedded content.

## Metadata Analysis

Inspect EXIF, timestamps, usernames, paths, logs, and metadata fields.

## Extraction

Explain carving, decompression, decoding, mounting, or recovery steps.

## Timeline / Artifacts

Summarize useful events, recovered files, indicators, and evidence.

## Solution

Explain the solving process step by step.

## Exploit / Solver

```python
# solver.py
```

## Flag

```text
flag{example}
```

## Lessons Learned

Write what you learned from this challenge.

## References

*
